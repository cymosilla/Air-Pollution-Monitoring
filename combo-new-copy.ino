...
struct pm25data {
  float low_ratio;             // percentage
  float concentration_mgm3;    // mg/m³
};
union {
  pm25data data;               // access as structured data
  uint8_t buffer[sizeof(pm25data)];  // access as byte array
} dbunion_pm25;
...
void setup() {
 ...
 Serial.println("Code is working");
  ...
  SCD4X.enablePeriodMeasure(SCD4X_STOP_PERIODIC_MEASURE); //commented to make it work

  SCD4X.setTempComp(4.0); //commented to make it work
  ...
  SCD4X.setSensorAltitude(540); //commented to make it work
  ...
  */
  ...
  SCD4X.enablePeriodMeasure(SCD4X_START_PERIODIC_MEASURE);  //commented to make it work
...
void readPMData() {
  ...
  float lowPulseDuration = 0;

  // Accumulate LOW pulses for the sample time
  while ((millis() - startTime) < sampleTime) {
    lowPulseDuration += pulseIn(pinPM25, LOW) / 1000.0;  // Convert µs to ms
  }

  // Calculate low ratio (%)
  float lowRatio = (lowPulseDuration / sampleTime) * 100.0;
  ...
  concentration = max(0.0f, concentration);

  dbunion_pm25.data.low_ratio = lowRatio;
  dbunion_pm25.data.concentration_mgm3 = concentration;

  // Store values in the global JSON document
  doc["pm25_low_ratio"] = dbunion_pm25.data.low_ratio;
  doc["pm25_concentration_mgm3"] = dbunion_pm25.data.concentration_mgm3;
}
